...
test("R2.1 catchBlast Upgrade Suite", async (t) => {
  ...
  await t.test("1. Single enemy within blast radius receives 1 damage", () => {
    ...
    assert.equal(enemy.hp, 2, "Enemy hp should decrease by 1");
  ...
  await t.test("2. Multiple enemies within blast radius all receive 1 damage", () => {
    ...
    for (let i = 0; i < enemies.length; i++) {
      assert.equal(enemies[i].hp, 2, `Enemy ${i} should have taken 1 damage`);
    }
  });
  ...
  await t.test("3. Enemy outside blast radius takes 0 damage", () => {
    ...
    assert.equal(farEnemy.hp, 3, "Far enemy should take 0 damage");
  });
  ...
  await t.test("4. Precise boundary condition (dist == blast + enemy.r vs dist > blast + enemy.r)", () => {
    ...
    assert.equal(edgeEnemy.hp, 2, "Edge enemy exactly at boundary <= blast+r should take 1 damage");
    assert.equal(outsideEnemy.hp, 3, "Outside enemy > blast+r should take 0 damage");
  });
  ...
  await t.test("5. Unspawned enemy (hasEnteredArena = false) is ignored", () => {
    ...
    assert.equal(unspawnedEnemy.hp, 3, "Unspawned enemy must take 0 damage");
  });
  ...
  await t.test("6. Zero stat behavior (catchBlast = 0 or undefined)", () => {
    ...
    assert.equal(closeEnemy.hp, 3, "Enemy takes 0 damage when catchBlast is 0");
  });
